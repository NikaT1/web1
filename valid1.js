function madeLight() {
    ...
    document.getElementById("imagine3").src = "pic3.png";
...
function madeDark() {
    ...
    document.getElementById("imagine3").src = "dark-pic3.png";
...
const url = "checkColor.php";
request.open("GET", url, true);
request.addEventListener("readystatechange", () => {
    if (request.readyState === 4 && request.status === 200) {
        if (request.response === "dark") madeDark();
        else madeLight();
    }
});
request.send();
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelector('#changeColor').addEventListener('click', function (e) {
        ...
        if (document.querySelector("body").classList.contains('dark-body')) {
            madeLight();
            const request = getXmlHttp();
            const url = "changeColor.php?color=light";
            request.open("GET", url, true);
            request.send();
        } else {
            madeDark();
            const request = getXmlHttp();
            const url = "changeColor.php?color=dark";
            request.open("GET", url, true);
            request.send();
        }
    });
    ...
    document.querySelector("#submit").addEventListener('click', function (e) {
        ...
        if (checkY() && checkX() && checkR()) {
            drawCoord();
            const request = new XMLHttpRequest();
            const url = "script.php?x=" + param_x + "&y=" + param_y + "&r=" + param_r + "&time=" + timeZone.getTimezoneOffset();
            request.open("GET", url, true);
            request.addEventListener("readystatechange", () => {
                if (request.readyState === 4 && request.status === 200) {
